...
def dice_loss(masks, labels, is_average=True):
    ...
    if is_average:
        ...
        return score
def dice_ratio(masks, labels, masks_id, is_average=True):
    """
    dice ratio
    :param masks:
    :param labels:
    ...
    """
    labels = labels.cpu()
    panc_masks, la_masks = masks[0].cpu(), masks[1].cpu()
    panc_id, la_id = masks_id[0], masks_id[1]
    panc_num = torch.sum(panc_id)
    la_num = torch.sum(la_id)
    
    if la_num == 0:
        m1 = panc_masks.flatten()
        ...
        m1_panc = panc_masks[panc_id].flatten()
        m2_panc = labels[panc_id].flatten().float()
        m1_la = la_masks[la_id].flatten()
        m2_la = labels[la_id].flatten().float()

        intersection_panc = m1_panc * m2_panc
        score_panc = (2 * intersection_panc.sum()) / (m1_panc.sum() + m2_panc.sum()+1e-6) 
        intersection_la = m1_la * m2_la
        score_la = (2 * intersection_la.sum()) / (m1_la.sum() + m2_la.sum()+1e-6)  
        score =  (score_panc + score_la) / 2.

    return score   

def uni_dice_ratio(masks, labels, is_average=True):
    """
    dice ratio
    :param masks:
    :param labels:
