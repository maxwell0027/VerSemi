...
def get_dataset_path(dataset='LA'):
...
class LAHeart(Dataset):
    ...
    def __init__(self, base_dir, dataset_name, split, no_crop=False, require_mask=False):
        ...
        data_list_paths = get_dataset_path(dataset_name)
        ...
        self.image_list = [os.path.join(self._base_dir, item+".h5") for item in self.image_list]
    ...
    def __getitem__(self, idx):
        ...
        if self.require_mask:
            ...
            return image_.float(), label_.long(), mask_.long()
        else:
            samples = image, label
            if self.transform:
                tr_samples = self.transform(samples)
